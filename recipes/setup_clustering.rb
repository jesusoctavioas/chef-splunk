...
cluster_secret = passwords['secret'] # this is the pass4symkey

Chef::Log.debug("Current node clustering mode: #{node['splunk']['clustering']['mode']}")

cluster_master_mgmt_uri = nil
unless cluster_master?
  search(
    ...
    cluster_master_mgmt_uri = result['cluster_master_mgmt_uri']
  end
end
...
when 'slave', 'searchhead'
  ...
  splunk_cmd_params <<
    " -master_uri #{cluster_master_mgmt_uri}" \
    " -replication_port #{node['splunk']['clustering']['replication_port']}"
else
  Chef::Log.fatal("You have set an incorrect clustering mode: #{node['splunk']['clustering']['mode']}")
